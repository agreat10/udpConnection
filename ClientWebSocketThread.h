...
struct bufferFromSocket
{
    std::mutex mutex;
    std::string data_;
};
bufferFromSocket bufferFromSocket_;
...
static int callback_echo(struct lws *wsi, enum lws_callback_reasons reason,
    ...
    switch (reason) {
        case LWS_CALLBACK_ESTABLISHED:
            ...
            SendMessage("Практика 2");
            break;
        case LWS_CALLBACK_RECEIVE:
            //lws_write(wsi, (unsigned char *)in, len, LWS_WRITE_TEXT);
            bufferFromSocket_.mutex.lock();
            bufferFromSocket_.data_ = std::string ((char *)in, len);
            bufferFromSocket_.mutex.unlock();
            break;
